...
DATA_INPUT: list[str] = ["day_ahead_auction"]
...
model = tf.keras.models.Sequential(
    [
        tf.keras.layers.InputLayer(input_shape=(WINDOW_SIZE, 1), batch_size=BATCHSIZE),
...
plot_predictions(
    eval_true, eval_pred, warmup=eval_dataset_df[:48]["day_ahead_auction"].to_list()
)
save_predictions(eval_pred, f"experiment_6_{WINDOW_SIZE}steps_ar", 250, 48)
...
save_predictions(eval_pred, f"experiment_6_{WINDOW_SIZE}steps_true", 250, 48)
